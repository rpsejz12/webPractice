...
<body class="is-preload">
	...
	<div id="wrapper">
		...
		<header id="header">
			...
			<div class="content">
				<div class="inner">
					<a href="control.jsp?action=mainA">전체목록보기</a>
					<c:forEach var="v" items="${datas}">
						<c:set var="m" value="${v.m}" />
						<h3>
							[${m.id}] ${m.msg} &gt;&gt; [좋아요 ${m.favcount} | 댓글
							${m.replycount} | ${m.mdate}]
							<c:if test="${m.id eq mem.id}">
								<a href="control.jsp?action=mdelete&mid=${m.mid}">&nbsp;&nbsp;게시글 삭제</a>
						...
						<ul>
							<c:forEach var="r" items="${v.rlist}">
								...
								<c:if test="${r.id eq mem.id}">
										<a href="control.jsp?action=rdelete&rid=${r.rid}">&nbsp;&nbsp;삭제</a>
									</c:if>
								</li>
							</c:forEach>

							<li>
							<a href="control.jsp?action=main&msgmid=${m.mid}&mcnt=${mcnt+3}">댓글 더보기</a> &nbsp;&nbsp;
							<c:if test="${m.mid ne param.rmid and mem.id ne null}">
							<a href="control.jsp?action=main&rmid=${m.mid}">댓글 등록</a>
							...
							<c:if test="${m.mid eq param.rmid}">
									...
									<input type="hidden" name="action" value="rinsert">
										...
										<table>
											<tr>
												<td><textarea name="rmsg" cols="40" rows="1" required></textarea></td>
		...
		<div id="main">
			...
			<article id="login">
				...
				<form action="control.jsp" method="post" name="form1">
					...
					<input type="hidden" name = "mcnt" value = "${param.mcnt+2}">
